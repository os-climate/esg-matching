...
from esgmatching.cleaner import simple_cleaner, cleaner_data
...
def get_pattern_name_to_clean():
    ...
    """
    return ['company_name', 'entity', 'name', 'company', 'owner_name']
...
def get_clean_name(company_name, normalize_legal_terms=True, output_lettercase='lower'):
    ...
    if not isinstance(company_name, str):
        raise AttributeError('Cleaning for company name cannot be applied to non string values.')
    ...
    clean_company_name = simple_cleaner.apply_cleaning_rules(clean_company_name, cleaning_dict)
